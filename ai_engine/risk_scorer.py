...
class RiskScorer:
    ...
    The scorer determines:
    - Severity level  : high / medium / low
    - Auto-isolate    : whether to isolate immediately or escalate to a human
    - Recommended actions : list of actions to take

    Usage:
        scorer = RiskScorer()
        result = await scorer.score(
            user_principal_name="jane@org.com",
            alert_reason="Impossible travel detected — login from two countries within 1 hour",
            sentinel_incident_id="INC-4821",
        )
    """
    ...
    async def score(
        ...
        """
        Score the risk level of a security alert.
        ...
        Raises:
            RiskScoringError: if scoring fails.
        """
    ...
    def _validate(self, result: dict) -> dict:
        """Validate and normalise the LLM risk scoring response."""

        severity_str = result.get("severity", "").lower()
        if severity_str not in ("high", "medium", "low"):
            raise RiskScoringError(
                f"LLM returned invalid severity: '{severity_str}'. Expected high/medium/low."
            )

        severity = RiskSeverity(severity_str)
        auto_isolate = bool(result.get("auto_isolate", False))

        # Safety override: high severity always auto-isolates regardless of LLM output
        if severity == RiskSeverity.HIGH:
            if not auto_isolate:
                logger.warning(
                    "LLM set auto_isolate=False for HIGH severity alert. "
